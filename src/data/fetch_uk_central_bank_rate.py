"""
Fetch UK central bank policy rate (Bank Rate) release history from FRED.

Series
------
    UK: IRSTCI01GBM156N — OECD "immediate rate" proxy for BoE Bank Rate, monthly.
    Medium confidence — monthly granularity vs. USA's daily DFF.
    Alternative: scrape bankofengland.co.uk directly for event-dated Bank Rate series.

Output
------
    data/raw/macro/UK_central_bank_rate.csv — columns: date, realtime_start, value.
    Raw; no forward-fill or transform (sqrt applied in process_central_bank_rate.py).

Usage
-----
    python fetch_uk_central_bank_rate.py --verify-only
    python fetch_uk_central_bank_rate.py --start 2013-01-01 --end 2024-12-31
"""
...
def main() -> None:
    ...
    log = StepLogger(TAG)
    log.warn(
        "IRSTCI01GBM156N is monthly (not daily like DFF) — BoE Bank Rate events between "
        "monthly dates will be forward-filled to the end of the month in process_central_bank_rate.py."
    )
    ...
    log.info(f"Series: {SERIES_ID} (monthly) | range: {args.start} → {end_str}")
