...
Series
------
    USA: IEABC — Balance on current account (BEA). Medium confidence.
    Run --verify-only to confirm series id and coverage before a full fetch.

Output
------
    data/raw/macro/USA_current_account.csv — columns: date, realtime_start, value.
    Raw; no forward-fill or transform (that's process_current_account.py).

Usage
-----
    python fetch_usa_current_account.py --verify-only
    python fetch_usa_current_account.py --start 2013-01-01 --end 2024-12-31
"""
...
def main() -> None:
    ...
    log.info(f"Series: {SERIES_ID} (quarterly) | range: {args.start} → {end_str}")
